...
# GlobalPixel bot version 1 by portasynthinca3 edited by cioz
...
def get_chunk(d, x, y):
    ...
    data = sess.get(f'https://globalpixel.xyz/chunks/{d}/{x}/{y}.bmp').content
...
def main():
    ...
    me = sess.get('https://globalpixel.xyz/api/me').json()
    ...
    if config.image.path == "":
        ...
        print(f'{Fore.YELLOW}Enter your GlobalPixel username or e-mail (leave empty to skip authorization): {Style.RESET_ALL}', end='')
        config.auth.login = input()
        config.auth.passwd = ''
        auth_token = ''
        if config.auth.login != '':
            config.auth.passwd = getpass.getpass(f'{Fore.YELLOW}Enter your GlobalPixel password: {Style.RESET_ALL}')
    ...
    if config.auth.login != '':
        ...
        response = sess.post('https://globalpixel.xyz/api/auth/local', json={
    ...
    def run_client():
        ...
        ws = websocket.create_connection('wss://globalpixel.xyz:443/ws', header=extra_ws_headers,
    ...
    while True:
        try:
            ...
            print(f'{Fore.RED}Interrupting for 1 Minute{Style.RESET_ALL}')
            time.sleep(60)
